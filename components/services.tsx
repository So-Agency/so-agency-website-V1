        ...
        <div className="space-y-4">
          ...
          <div ref={gridRef} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {services.slice(0, 2).map((service, index) => (
              <ScrollReveal key={service.title} delay={index * 100}>
                <TiltCard
                  className={`relative group rounded-2xl border border-white/10 bg-white/5 backdrop-blur-md p-6 transition-colors duration-300 card-shine overflow-hidden ${
                    service.disabled 
                      ? "opacity-60" 
                      : `hover:border-[#FEC700]/50 hover:bg-white/10 ${service.borderGlow}`
                  }`}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {services.slice(2, 5).map((service, index) => (
              <ScrollReveal key={service.title} delay={index * 100}>
                <TiltCard
                  className={`relative group rounded-2xl border border-white/10 bg-white/5 backdrop-blur-md p-6 transition-colors duration-300 card-shine overflow-hidden h-full ${
                    service.disabled 
                      ? "opacity-60" 
                      : `hover:border-[#FEC700]/50 hover:bg-white/10 ${service.borderGlow}`
                  }`}
